...
def plot_bargraphs(data):
    ...
    metrics = ['Throughput(MH/s)', 'PerformanceTime(sec)', 'DataTransferRate(MB/s)']
    ...
    for metric in thread_types:
        ...
        image_path = os.path.join(output_dir, f'1GB_{metric.replace("/", "-")}_bargraph.png')
        plt.savefig(image_path)
        plt.show()

def main():
    # Input file path
    path = os.path.join('../input', 'configurations_time_1gb.txt')
    # Read the data
    data = read_data(path)
